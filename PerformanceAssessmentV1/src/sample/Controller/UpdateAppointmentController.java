...
public class UpdateAppointmentController implements Initializable {
    @FXML
    private Button UpdateAppointmentCreateButton;
    @FXML
    private Button UpdateAppointmentBackButton;
    @FXML
    ...
    @FXML
    private TextField UpdateAppointmentTypeTextField;
    @FXML
    private ComboBox<Contacts> UpdateAppointmentContactComboBox;
    @FXML
    private DatePicker UpdateAppointmentStartDatePicker;
    @FXML
    private DatePicker UpdateAppointmentEndDatePicker;
    @FXML
    private ComboBox UpdateAppointmentStartComboBox;
    @FXML
    private ComboBox UpdateAppointmentEndComboBox;
    @FXML
    private ComboBox<Integer> UpdateAppointmentCustomerComboBox;
    @FXML
    private ComboBox<Integer> UpdateAppointmentUserComboBox;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        UpdateAppointmentContactComboBox.setItems(contacts);
        ...
        while(startBusiness.isBefore(endBusiness.plusSeconds(1))){
            UpdateAppointmentStartComboBox.getItems().add(startBusiness);
            UpdateAppointmentEndComboBox.getItems().add(startBusiness);
        ...
        for(Customers c: customers){
            UpdateAppointmentCustomerComboBox.getItems().add(c.getCustomerID());
        }
        for(Users u: users){
            UpdateAppointmentUserComboBox.getItems().add(u.getUserID());
        }
    }
    ...
    private boolean timeChecker(LocalDateTime st, LocalDateTime en){
        ...
        for(Appointments a : appointments) {
            if(a.getCustomerID() == (int) UpdateAppointmentCustomerComboBox.getSelectionModel().getSelectedItem()
            && a.getAppointmentID() != Integer.parseInt(UpdateAppointmentIDTextField.getText())) {
    ...
    public void updateAppointmentButtonHandler(ActionEvent e) {
        try {
            int aID = Integer.parseInt(UpdateAppointmentIDTextField.getText());
            String aTitle = UpdateAppointmentTitleTextField.getText();
            String aDescription = UpdateAppointmentDescriptionTextField.getText();
            String aLocation = UpdateAppointmentLocationTextField.getText();
            String aType = UpdateAppointmentTypeTextField.getText();
            int custID = UpdateAppointmentCustomerComboBox.getSelectionModel().getSelectedItem();
            int uID = UpdateAppointmentUserComboBox.getSelectionModel().getSelectedItem();
            int conID = UpdateAppointmentContactComboBox.getSelectionModel().getSelectedItem().getContactID();
            LocalTime startTime = LocalTime.parse(UpdateAppointmentStartComboBox.getSelectionModel().getSelectedItem().toString());
            LocalTime endTime = LocalTime.parse(UpdateAppointmentEndComboBox.getSelectionModel().getSelectedItem().toString());
            LocalDate startDate = UpdateAppointmentStartDatePicker.getValue();
            LocalDate endDate = UpdateAppointmentEndDatePicker.getValue();
            ...
            LocalDateTime en = ZonedDateTime.of(end, ZoneId.of("UTC")).toLocalDateTime();
                ...
                if (timeChecker(start, end)) {
                    DBAppointments.updateAppointment(aID, aTitle, aDescription, aLocation, aType, st, en, custID, uID, conID);
                    try {
                        URL url = new File("src/sample/View/Appointments.fxml").toURI().toURL();
                        Parent root = FXMLLoader.load(url);
                        Scene CustomersScreen = new Scene(root, 800, 375);
    ...
    public void sendAppointmentData(Appointments a) {
        UpdateAppointmentIDTextField.setText(Integer.toString(a.getAppointmentID()));
        UpdateAppointmentTitleTextField.setText(a.getAppointmentTitle());
        UpdateAppointmentDescriptionTextField.setText(a.getAppointmentDescription());
        UpdateAppointmentLocationTextField.setText(a.getAppointmentLocation());
        UpdateAppointmentTypeTextField.setText(a.getAppointmentType());
        UpdateAppointmentContactComboBox.setValue(DBContacts.getContact(a.getContactID()));
        UpdateAppointmentCustomerComboBox.setValue(a.getCustomerID());
        UpdateAppointmentUserComboBox.setValue(a.getUserID());
        UpdateAppointmentStartDatePicker.setValue(a.getStart().toLocalDate());
        UpdateAppointmentEndDatePicker.setValue(a.getEnd().toLocalDate());
        UpdateAppointmentStartComboBox.setValue(a.getStart().toLocalTime());
        UpdateAppointmentEndComboBox.setValue(a.getEnd().toLocalTime());
    }
}
